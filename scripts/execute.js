// Execute a tallied governance proposal via X proof
// Usage: node execute.js <token> <vault> <proposalId> <tweetId> <xHandle> <xId>
// Example: node execute.js 0xtoken 0xvault 42 1234567890 andi 9876543210
...
async function main() {
  ...
  if (args.length < 6) {
    fail("Usage: execute.js <token> <vault> <proposalId> <tweetId> <xHandle> <xId>");
  }

  const [token, vault, proposalId, tweetId, xHandle, xId] = args;
  ...
  // Canonical substring
  const substring = `execute proposal ${token} ${vault} ${proposalId}`;
  ...
  // Submit
  const tx = await vaultContract.executeProposalByProof(propId, proof, proofData.signature);
